...
- [ ] Fix screen header rules.
  ...
  - [ ] Today uses the compact header from Figma Make: back on left, small brand on right.
  - [ ] Recovery reasons and plan approval use the back button/title block layout, not a centered app header.
  - [ ] Do not add a header where the Figma Make screen does not have one.
- [ ] Rebuild the chat screen to match the initial published state.
  - [ ] Initial chat shows only the onboarding-based welcome bubble.
  - [ ] Remove fake multi-message conversation from the initial state unless it is generated by user action.
  - [ ] Preserve the large empty chat canvas and docked input from the reference.
  - [ ] Replace visible AI wording with product language while keeping the same layout.
- [ ] Rebuild the chat proposal card.
  - [ ] Match the dashed proposal header, icon, type label, item rows, footnote, dismiss button, and approve button.
  - [ ] Use the Figma Make two-button action layout.
  - [ ] Do not mutate the plan before approval.
