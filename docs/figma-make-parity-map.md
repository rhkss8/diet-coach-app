...
| `onboarding`       | `OnboardingScreen`                   | `apps/mobile/src/features/onboarding/OnboardingFlow.tsx`, `BasicProfileStep.tsx`, `GoalSetupStep.tsx`, `LifestyleStep.tsx` | Intentional exception | Figma Make has one compact onboarding screen, but the tester MVP keeps chat-first guest entry. Basic body metrics are collected through the plan-basis settings gate, and planning context is collected through the first-plan preparation card. Do not restore the old onboarding route only for visual parity unless the chat-first tester readout fails. |
| `chat`             | `ChatScreen`                         | `apps/mobile/src/features/consultation/ConsultationChatScreen.tsx`, `apps/mobile/src/root/AppRoot.tsx`                     | Implemented           | RN has the bordered top bar, assistant bubble, large empty canvas, docked input, first-plan preparation card, and quick actions. The "오늘 플랜" action is hidden until an approved plan exists, preserving the initial right spacer.                                                                                                                       |
| `chat-proposal`    | `ChatProposalScreen`, `ProposalCard` | `ConsultationChatScreen.tsx`, `PlanProposalCard` in `apps/mobile/src/shared/ui/planner-components.tsx`                     | Implemented           | RN has the proposal card shell, dashed header, icon, type label, two-line item rows, footnote, dismiss action, and approve action. Shared proposal icons use lucide `Leaf` and `Check`, and plan mutation remains behind explicit approval.                                                                                                                 |
...
- Onboarding is an intentional parity exception. The tester MVP uses chat-first guest entry, plan-basis settings, and the first-plan preparation card instead of restoring Figma Make's single onboarding screen.
- Visual QA captured the currently routable states: login, chat, chat-proposal, today, recovery-reasons, and plan-approval.
- The onboarding exception is documented in `docs/decision-log.md`; revisit it only after tester feedback says chat-first entry lacks enough profile context.

## Recommended Next Slice

Move back to tester recruitment:

1. Confirm the tester distribution path.
2. Confirm the feedback URL.
3. Fill the first tester list.
