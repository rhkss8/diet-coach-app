...
1. Keep only the onboarding-based welcome bubble in the initial chat state.
2. Remove fake multi-message conversation from initial state unless generated by user action.
3. Preserve the large empty chat canvas and docked input from the reference.
4. Replace visible AI wording with product language while keeping the same layout.
